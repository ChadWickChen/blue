...
@property (nonatomic, strong) NSArray *childImgs;
@property (nonatomic, strong) NSArray *childSelectImgs;
...
-(instancetype)initWithChildControllers:(NSArray<UIViewController *> *)childControllers childTitles:(NSArray<NSString *> *)childTitles childImgs:(NSArray<NSString *> *)childImgs childSelectImgs:(NSArray<NSString *> *)childSelectImgs
{
    self = [super init];
    if(self) {
        _childControllers = childControllers;
        _childTitles = childTitles;
        _childImgs=childImgs;
        _childSelectImgs=childSelectImgs;
...
//            XXPageTabItemLable *tabItem = (XXPageTabItemLable *)_tabItems[i];
            CDPageTabItemButton*tabItem=(CDPageTabItemButton *)_tabItems[i];
...
//        [_tabItems addObject:tabItem];
        ...
        CDPageTabItemButton* tabItem=[[CDPageTabItemButton alloc]init];
        tabItem.titleLabel.font=_tabItemFont;
        [tabItem setTitle:_childTitles[i] forState:(UIControlStateNormal)];
        [tabItem setTitleColor:_unSelectedColor forState:(UIControlStateNormal)];
        [tabItem setTitleColor:_selectedColor forState:(UIControlStateSelected)];
        [tabItem setImage:[UIImage imageNamed:_childSelectImgs[i]] forState:(UIControlStateSelected)];
        [tabItem setImage:[UIImage imageNamed:_childImgs[i]] forState:(UIControlStateNormal)];
        tabItem.selected=i==_selectedTabIndex?YES:NO;
        [tabItem layoutButtonWithEdgeInsetsStyle:MKButtonEdgeInsetsStyleLeft imageTitleSpace:5];
        [tabItem addTarget:self action:@selector(changeChildControllerOnClick:) forControlEvents:(UIControlEventTouchUpInside)];
        [_tabItems addObject:tabItem];
...
//    XXPageTabItemLable *currentTabItem = _tabItems[_selectedTabIndex];
    CDPageTabItemButton* currentTabItem = _tabItems[_selectedTabIndex];
...
//    nextTabItem.textColor = _selectedColor;
    ...
    CDPageTabItemButton *currentTabItem = _tabItems[_selectedTabIndex];
    CDPageTabItemButton *nextTabItem = _tabItems[nextIndex];
    currentTabItem.selected=NO;
    nextTabItem.selected=YES;
    [currentTabItem setTitleColor:_unSelectedColor forState:(UIControlStateNormal)];
    [nextTabItem setTitleColor:_selectedColor forState:(UIControlStateSelected)];
...
//        XXPageTabItemLable *tabItem = _tabItems[i];
        CDPageTabItemButton *tabItem = _tabItems[i];
...
//    XXPageTabItemLable *selecedTabItem = _tabItems[_selectedTabIndex];
    CDPageTabItemButton *selecedTabItem = _tabItems[_selectedTabIndex];
...
- (void)changeChildControllerOnClick:(UIButton *)tap {
    NSInteger nextIndex = [_tabItems indexOfObject:tap];
    if(nextIndex != _selectedTabIndex) {
        if(_titleStyle == XXPageTabTitleStyleDefault) {
            [self changeSelectedItemToNextItem:nextIndex];
        }
...
//        rightTabItem.textColor = rightItemColor;
        ...
        CDPageTabItemButton *leftTabItem = _tabItems[_leftItemIndex];
        CDPageTabItemButton *rightTabItem = _tabItems[_rightItemIndex];
        [leftTabItem setTitleColor:leftItemColor forState:(UIControlStateNormal)];
        [rightTabItem setTitleColor:rightItemColor forState:(UIControlStateNormal)];
        [leftTabItem setTitleColor:leftItemColor forState:(UIControlStateSelected)];
        [rightTabItem setTitleColor:rightItemColor forState:(UIControlStateSelected)];
...
//        tabItem.textColor = i==_selectedTabIndex?_selectedColor:_unSelectedColor;
        CDPageTabItemButton *tabItem = _tabItems[i];
        [tabItem setTitleColor:i==_selectedTabIndex?_selectedColor:_unSelectedColor forState:(UIControlStateNormal)];
    }
...
//    tabItem.textColor = _selectedColor;
    CDPageTabItemButton *tabItem = _tabItems[_selectedTabIndex];
    [tabItem setTitleColor:_selectedColor forState:(UIControlStateSelected)];
