...
#pragma mark 显示信息
+ (void)CD_show:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
    if (view == nil) {
...
+ (void)CD_showError:(NSString *)error toView:(UIView *)view{
    [self CD_show:error icon:@"error.png" view:view];
}

#pragma mark 显示正确信息
+ (void)CD_showSuccess:(NSString *)success toView:(UIView *)view
{
    [self CD_show:success icon:@"success.png" view:view];
}
